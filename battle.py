...
class Game:
    ...
    def greet(self):
        ...
        print("x - номер строки  ;  y - номер столбца.")
                                ...
                                #Выводим две доски игрока и компьютера параллельно
    @staticmethod
    def parallel(board_us, board_ai):
        board_us = board_us.split("\n")
        board_ai = board_ai.split("\n")
        maxlen = max(map(len, board_us))
        result = ""
        for line_1, line_2 in zip(board_us,board_ai):
            result += f"{line_1:{maxlen}}    |    {line_2}\n"
        return result
    ...
    def loop(self):
        ...
        while True:
            ...
            print("-"*70)
            print(Game.parallel(user_output, ai_output))
            if num % 2 == 0:
                print("-" * 70)
